...
Section Interleave.
  ...
  (* All the solutions above interleave two sequential computations, and introduce a scheduler in the process.
     ...
     TODO: Écrire la version où tout non-tau commence par un sched qui donne la main à droite ou à gauche.

Structural Operational Semantics (SOS)

Deux calculs séquentiels en parallèle donnent un cacul "concurrent"

Deux calculs concurrents en parallèle ?
...
[Sched1 (true -> Vis a;; Vis b ,
        false -> Vis b ;; Vis a)]
 || [Vis c]

Sched1 (true  -> (Vis a;; Vis b || Vis c) ,
        false -> (Vis b;; Vis a || Vis c))

Sched1 (true -> (Sched2tt
                 (true -> Vis a ;; (Vis b || Vis c)
                  false -> Vis c ;; Vis a ;; Vis b)
        false -> b a || c

Sched2tt
 vrai -> Sched 1 (Vis a ;; vis b  || vis c) ()
 false -> Vis c;;

Tau^omega || (Sched; Vis a) ~~ Tau^omega

Tau^omega ||  ~~
 Sched (true -> Tau^omega | false -> Sched (
...
Tau^omega
  ...
  Vis A   Vis B    Vis C Vis D
           ...
           Sched
    Sched           Sched
Vis A   Vis C   Vis B   Vis C
   |       | 
Vis C   Vis A   Vis C   Vis B
 ...
 Sched   C
  ...
  Definition interleave''' :
    itree (SchedE +' E1) X1 -> itree (SchedE +' E2) X2
    -> itree (SchedE +' E1 +' E2) (X1 * X2) :=
...
(* Could alternatively parameterize the handler by a scheduler
   given by a stream of bits and quantify at the meta level over
   all schedules
...
t1 interp (sched true) ~ a
t1 interp (sched false) ~ b

Choice3

 *)

(* Note: a benefit of having a deterministic handler parameterized by
   a schedule with a meta quantification is that we can easily restrict
   the set of schedulers we quantify over.
   Typically, one may want to consider only fair schedulers, but such a
   restriction is intrinsically global (unless we were to fix an upper
   bound on the fairness naturally), and hence difficult if possible at
   all to internalize.
 *)
